...
# (b)
print('executing Q5(b)')
_training = np.hstack([X_train,y_train.reshape(-1,1)])
acc_dict = {
    'Decision Trees':[],
    'Random Forest': [],
    'AdaBoost':[],
...
for name, clf in classifiers.items():
    acc=[]
    for t in range(10):
        _subset=_training[np.random.choice(len(_training), size)]
...
plt.plot(sizes,acc_dict['Decision Trees'],'b',label='Decision Trees')
plt.plot(sizes,acc_dict['Random Forest'],'orange', label='Random Forest')
plt.plot(sizes,acc_dict['AdaBoost'],'g', label='AdaBoost')
plt.plot(sizes,acc_dict['Logistic Regression'],'red',label='Logistic Regression')
plt.plot(sizes,acc_dict['MLP'],'m',label='MLP')
plt.plot(sizes,acc_dict['SVM Classifier'],'y',label='SVM Classifier')
plt.legend(loc=4)
plt.title('Performance')
plt.savefig('./pic/Performance.png')

# (c)
print('executing Q5(c)')
_training = np.hstack([X_train,y_train.reshape(-1,1)])
time_dict = {
    'Decision Trees':[],
    'Random Forest': [],
...
plt.figure()
sizes = ['50', '100', '200', '300', '400', '500', '600', '700', '800', '900','1000']
plt.plot(sizes,time_dict['Decision Trees'],'b',label='Decision Trees')
plt.plot(sizes,time_dict['Random Forest'],'orange', label='Random Forest')
plt.plot(sizes,time_dict['AdaBoost'],'g', label='AdaBoost')
plt.plot(sizes,time_dict['Logistic Regression'],'red',label='Logistic Regression')
plt.plot(sizes,time_dict['MLP'],'m',label='MLP')
plt.plot(sizes,time_dict['SVM Classifier'],'y',label='SVM Classifier')
plt.legend(loc='best')
plt.title('Training time (secs)')
plt.savefig('./pic/Time.png')